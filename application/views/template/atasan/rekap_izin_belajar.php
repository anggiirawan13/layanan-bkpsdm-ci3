...
<script type="text/javascript" language="javascript">
	function tampilkan() {
		...
		$.ajax({
			url: '<?php echo base_url("AtasanController/get_rekap_bulanan_izin_belajar") ?>',
			type: "GET",
			data: postData,
			dataType: "JSON",
			...
			success: function(result) {
				...
				for (var i = 0; i < result.length; i++) {
					var berkas = base_url + "assets/img/" + result[i]['dok_ijazah_terakhir'];
					var berkas1 = base_url + "assets/img/" + result[i]['dok_surat_humdis'];
					var berkas2 = base_url + "assets/img/" + result[i]['dok_surat_dinas'];
					...
					if (result[i]['status_pengajuan'] === "DITERIMA") {
						btn_tampil = btn1;
					} else if (result[i]['status_pengajuan'] === "DITOLAK") {
						btn_tampil = btn2;
					}
					...
					t.row.add([
						...
						"<center>" + result[i]['unit_kerja'] + "</center>",
						"<center>" + result[i]['nama_instansi_pendidikan'] + "</center>",
						"<center>" + result[i]['jenjang_pendidikan'] + "</center>",
						"<center>" + result[i]['status_pengajuan'] + "</center>",
						"<center>" + result[i]['tgl_pengajuan'] + "</center>",
	...
	function cetak_rekap() {
		...
		pendidikan = $('#pendidikan').val() == "" ? "all" : $('#pendidikan').val()
	...
	$('#rekap_all_izin_belajar').ready(function() {
		...
		function load_data() {
			$.ajax({
				url: '<?php echo site_url("AtasanController/get_all_rekap_izin_belajar") ?>',
				dataType: "JSON",
				success: function(result) {
					c.clear().draw();
					var HTMLbuilder = "";
